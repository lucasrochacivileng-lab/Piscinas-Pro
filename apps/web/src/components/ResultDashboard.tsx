...
export function ResultDashboard({ result }: { result: Phase1DesignResult }) {
  ...
  const hasSlope = result.geometryModel?.hasSlopedFloor ?? false;
  return <section className="results-panel">
    ...
    <div className="metrics">
      ...
      <article><small>Perfil do fundo</small><strong>{hasSlope ? "Com praia inclinada" : `${result.hydrostatic.zones?.length ?? 1} zona(s)`}</strong></article>
      <article><small>Paredes calculadas</small><strong>{wallPanels.length || 4}</strong></article>
    </div>
    {result.hydrostatic.zones && <><h3>Zonas de profundidade</h3><div className="reinforcement-grid">
      {result.hydrostatic.zones.map((zone) => <article key={zone.id}><h3>{zone.label}</h3><p>{format(zone.lengthMm, 0)} mm de comprimento horizontal</p>{zone.floorProfile === "SLOPED" ? <><p>Profundidade: {format(zone.startWaterDepthMm, 0)} → {format(zone.endWaterDepthMm, 0)} mm</p><p>Inclinação: {format(zone.slopePercent, 2)}% · piso real {format(zone.floorLengthMm, 0)} mm</p><p>Pressão no piso: {format(zone.startFloorPressureKPa)} → {format(zone.endFloorPressureKPa)} kPa</p></> : <p>{format(zone.waterDepthMm, 0)} mm de profundidade</p>}<p>{format(zone.volumeM3, 2)} m³ de água</p></article>)}
    </div></>}
