...
export function ResultDashboard({ result }: { result: Phase1DesignResult }) {
  return <section className="results-panel">
    ...
    <h3>Verificações</h3><div className="checks-list">{result.checks.map((check) => <div className="check-row" key={check.id}><StatusBadge status={check.status} /><span>{check.message}</span></div>)}</div>
    {result.warnings.length > 0 && <div className="warning-box"><strong>Atenção técnica</strong>{result.warnings.map((warning) => <p key={warning}>{warning}</p>)}</div>}
  </section>;
}
